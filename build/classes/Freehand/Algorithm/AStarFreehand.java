...
public class AStarFreehand  {
    ...
    public AStarFreehand(AStarFreehand precedent, Node startingNode, Node endingNode, WritableImage g) {
        ...
        while (!openSet.isEmpty() && current.equals(endingNode) == false) {
     ...
     * @param precedent The precedent AStarFreehand object initialised with all its Collections. In an early stage of development, we used to copy all this and pass them to the next AStarFreehand object
     */
    public AStarFreehand(AStarFreehand precedent, Node startingNode, Node endingNode, ArrayList<ArrayList<Boolean>> g, WritableImage img) {

        this.endingNode = endingNode;
       // this.endingNode.setX(this.endingNode.getX()/10);
        //this.endingNode.setY(this.endingNode.getY()/10);
        //We initiate all the useful sets
        ...
        //current.setX(current.getX()/10);
        //current.setY(current.getY()/10);
        //While there is still a Node to evaluate, the while is not over
        while (!openSet.isEmpty() && heuristicEstimation(current,endingNode) != 0) {
            //We, firstly, will get the unevaluated node with the lowest score in the nodeTotalScore Map
            current = null;
            Double minScore = Double.MAX_VALUE;
            for (Node n : openSet) {
           ...
           if (heuristicEstimation(current,endingNode) == 0) {
               this.solution = AStar.solved(efficientPredecessor, endingNode, new ArrayList<>());
               break;
           }
           //After that, we pass it into the closed set as it has been treated
           ...
           closedSet.add(current);
           //Then, we look at all its neighbours to seek for a potential candidate
           for (Node n : getNeighboursFromImage(g, current, img)) {
               if (!closedSet.contains(n) && !precedent.getSolution().contains(n)) {
                   if (!openSet.contains(n)) {
                       openSet.add(n);
                   }
     ...
     * @return A list of all neighbours
     */
    public ArrayList<Node> getNeighboursFromImage(ArrayList<ArrayList<Boolean>> g, Node current, WritableImage img) {
        ArrayList<Node> res = new ArrayList<Node>();
        PixelWriter pw = img.getPixelWriter();
        //left
        //if ( current.getX()-1-this.getEndingNode().getX() > 0  ) {
            if ((current.getX()-1>0)) { 
                if (g.get(current.getY()).get(current.getX()-1)) {
                    for (int x = 0; x < 10; x++) {
                        for (int y = 0; y < 10; y++) {
                                pw.setColor(10*(current.getX()-1)+x, 10*(current.getY())+y, Color.GREEN);
                        }
                    }                
                    res.add(new Node(current.getX()-1, current.getY()));
                } else {
                    for (int x = 0; x < 10; x++) {
                        for (int y = 0; y < 10; y++) {
                                pw.setColor(10*(current.getX()-1)+x, 10*(current.getY())+y, Color.BLACK);
                        }
                    }
                }
            }
        ...
        //right neighbour
        //if ( current.getX()+1-this.getEndingNode().getX() < 0 ) {
            if ((current.getX()+1<g.get(current.getY()).size())) {
                    if (g.get(current.getY()).get(current.getX()+1)) {
                    for (int x = 0; x < 10; x++) {
                        for (int y = 0; y < 10; y++) {
                                pw.setColor( 10*(current.getX()+1)+y,10*(current.getY())+x,Color.RED);
                        }
                    } 
                    res.add(new Node(current.getX()+1, current.getY()));
                } else {
                    for (int x = 0; x < 10; x++) {
                        for (int y = 0; y < 10; y++) {
                                pw.setColor(10*(current.getX()+1)+y,10*(current.getY())+x, Color.BLACK);
                        }
                    }
                }
            }
        ...
        //if ( current.getY()-1-this.getEndingNode().getY() > 0 ) {
            if ((current.getY()-1>0)) { 
                if ((g.get(current.getY()-1).get(current.getX()))) {
                for (int x = 0; x < 10; x++) {
                    for (int y = 0; y < 10; y++) {
                            pw.setColor(10*(current.getX())+x, 10*(current.getY()-1)+y, Color.WHITE);
                    }
                }
                res.add(new Node(current.getX(), current.getY()-1));
            }  else {
                for (int x = 0; x < 10; x++) {
                    for (int y = 0; y < 10; y++) {
                            pw.setColor(10*(current.getX())+x, 10*(current.getY()-1)+y, Color.BLACK);
                    }
                }
            }
            }
        ...
        //bottom neighbour
        //if ( current.getY()+1-this.getEndingNode().getY() < 0 ) {
            if (current.getY()+1<g.size()) { 
                if ((g.get(current.getY()+1).get(current.getX()))) {
                for (int x = 0; x < 10; x++) {
                    for (int y = 0; y < 10; y++) {
                            pw.setColor(10*(current.getX())+x, 10*(current.getY()+1)+y, Color.BLUE);
                    }
                }
                res.add(new Node(current.getX(), current.getY()+1));
            }  else {
